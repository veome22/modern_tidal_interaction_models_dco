...
def compute_fim_in_batches(net, det_events, batch_size=50, res=500):
    """
    Compute Fisher matrices in batches to avoid OOM.
    
    Parameters
    ----------
    net        : gwfast Network object
    det_events : dict of arrays, shape (N_det,) each
    batch_size : number of events per batch (tune down if still OOM)
    res        : frequency grid resolution (tune down if still OOM)
    
    Returns
    -------
    FIM_all : np.ndarray, shape (n_params, n_params, N_det)
    """
    N_det = len(next(iter(det_events.values())))
    n_batches = int(np.ceil(N_det / batch_size))
    FIM_batches = []

    for i in range(n_batches):
        lo = i * batch_size
        hi = min(lo + batch_size, N_det)
        print(f"    FIM batch {i+1}/{n_batches}  (events {lo}–{hi-1})", flush=True)

        batch = {k: v[lo:hi] for k, v in det_events.items()}
        fim_batch = net.FisherMatr(batch, res=res)   # (n_params, n_params, batch_size)
        FIM_batches.append(fim_batch)

    return np.concatenate(FIM_batches, axis=-1)      # (n_params, n_params, N_det)
