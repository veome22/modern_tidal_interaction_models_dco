...
from COMPAS.compas_python_utils.cosmic_integration import ClassCOMPAS
from COMPAS.compas_python_utils.cosmic_integration import selection_effects
import warnings
import astropy.units as u
import argparse
import importlib
from COMPAS.compas_python_utils.cosmic_integration.cosmology import get_cosmology
import pickle
...
pop_paths = {'notides': '../pop_sims/notides_pop_v329_combined.h5',            
...
# dco_types = ["BHBH", "BHNS", "BNS"]
dco_types = ["BHBH"]
dco_type_sums = {"BBH": 28, "BHNS": 27, "BNS": 26}
...
def compute_snr_and_detection_grids(sensitivity="O3b", snr_threshold=8.0, Mc_max=300.0, Mc_step=0.1,
...
def find_detection_rate(path, dco_type="BBH", merger_output_filename=None, weight_column=None,
                        ...
                        min_logZ=-12.0, max_logZ=0.0, step_logZ=0.01,
                        sensitivity="O3b", snr_threshold=8, 
...
def append_rates(path, detection_rate, formation_rate, merger_rate, redshifts, COMPAS, n_redshifts_detection,
    maxz=1., sensitivity="O3b", dco_type="BHBH", mu0=0.035, muz=-0.23, sigma0=0.39, sigmaz=0., alpha=0.,
...
for dco_type in dco_types:
    ...
    for pop_name in pop_names: 
        ...
        # Calculate the representative SF mass
        Average_SF_mass_needed = (COMPAS.mass_evolved_per_binary * COMPAS.n_systems)
        print(pop_name, 'Average_SF_mass_needed = ', Average_SF_mass_needed) # print this, because it might come in handy to know when writing up results :)
        n_formed = sfr / Average_SF_mass_needed # Divide the star formation rate density by the representative SF mass
        ...
        N_DCO = formation_rate.shape[0]
    
        bootstrapped_formation_rates = np.zeros((N_BOOT, len(redshifts)))
        bootstrapped_merger_rates = np.zeros((N_BOOT, len(redshifts)))
        bootstrapped_detection_rates = np.zeros((N_BOOT, n_redshifts_detection))

        for i in range(N_BOOT):
            boot_indices = np.random.choice(N_DCO, N_DCO, replace=True)

            bootstrapped_formation_rates[i] = np.sum(formation_rate[boot_indices], axis=0)
            bootstrapped_merger_rates[i] = np.sum(merger_rate[boot_indices], axis=0)
            bootstrapped_detection_rates[i] = np.sum(detection_rate[boot_indices], axis=0)

            if i==0: label=pop_labels[pop_name]
            else: label=None

            if i%10==0:
                print(f"{i} out of N_BOOT={N_BOOT} bootstraps done")
#                 axes[0].plot(redshifts, bootstrapped_formation_rates[i], color=pop_colors[pop_name], lw=0.3, label=label, alpha=0.2)
#                 axes[1].plot(redshifts, bootstrapped_merger_rates[i], color=pop_colors[pop_name], lw=0.3, label=label, alpha=0.2)
#                 axes[2].plot(redshifts[:n_redshifts_detection], bootstrapped_detection_rates[i], color=pop_colors[pop_name], lw=0.3, label=label, alpha=0.2)
        
        low_med_high = np.sum(np.percentile(bootstrapped_detection_rates, [2.5, 50, 97.5], axis=0), axis=1)
        print(f"{dco_type}, {pop_labels[pop_name]} Detection rate (2.5%, 50%, 97.5%): {low_med_high}")
        
        low_med_high = np.sum(np.percentile(bootstrapped_merger_rates[:, :n_redshifts_detection], [2.5, 50, 97.5], axis=0), axis=1)
        print(f"{dco_type}, {pop_labels[pop_name]} Merger rate out to z={redshifts[n_redshifts_detection-1]:.2f} (2.5%, 50%, 97.5%): {low_med_high}")
        
        
        # Formation Rate
        axes[0].plot(redshifts, np.median(bootstrapped_formation_rates, axis=0), color=pop_colors[pop_name], label=pop_labels[pop_name], lw=3)
        axes[0].fill_between(redshifts, np.percentile(bootstrapped_formation_rates, 2.5, axis=0), np.percentile(bootstrapped_formation_rates, 97.5, axis=0), color=pop_colors[pop_name], alpha=0.3)

        # Merger Rate
        axes[1].plot(redshifts, np.median(bootstrapped_merger_rates, axis=0), color=pop_colors[pop_name], label=pop_labels[pop_name], lw=3)
        axes[1].fill_between(redshifts, np.percentile(bootstrapped_merger_rates, 2.5, axis=0), np.percentile(bootstrapped_merger_rates, 97.5, axis=0), color=pop_colors[pop_name], alpha=0.3)

        # Detection Rate
        axes[2].plot(redshifts[:n_redshifts_detection], np.median(bootstrapped_detection_rates, axis=0), color=pop_colors[pop_name], label=pop_labels[pop_name], lw=3)
        axes[2].fill_between(redshifts[:n_redshifts_detection], np.percentile(bootstrapped_detection_rates, 2.5, axis=0), np.percentile(bootstrapped_detection_rates, 97.5, axis=0), color=pop_colors[pop_name], alpha=0.3)
            
    axes[0].set_ylabel(r'$\rm \frac{\mathrm{d}N_{\rm Form}}{\mathrm{d}Gpc^3 \mathrm{d}yr} [Gpc^{-3} yr^{-1}]$', fontsize=fs+2)
    axes[1].set_ylabel(r'$\rm \frac{\mathrm{d}N_{\rm Merge}}{\mathrm{d}Gpc^3 \mathrm{d}yr} [Gpc^{-3} yr^{-1}]$', fontsize=fs+2)
    axes[2].set_ylabel(r'$\rm \frac{\mathrm{d}N_{\rm Det}}{\mathrm{d}Gpc^3 \mathrm{d}yr} [Gpc^{-3} yr^{-1}]$', fontsize=fs+2)    
    axes[2].set_xlim(0, 0.29)

    for ax in axes.flatten():
        ax.set_xlabel('Redshift', fontsize=fs)
        ax.tick_params(axis='both', which='major', labelsize=fs_axis)
        leg = ax.legend(fontsize=fs-2)
        for line in leg.get_lines():
            line.set_linewidth(3.0)

    # plt.suptitle(dco_type, fontsize=fs)
    plt.tight_layout()
    plt.savefig(plot_path + f'{dco_type}_bootstrapped_rates.pdf', dpi=300, bbox_inches='tight')
